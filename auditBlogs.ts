...
while (true) {
  ...
  const authIndex = contentBlock.search(
  ...
  if (authIndex !== -1) {
    ...
    if (listStartIndex !== -1) {
      ...
      let listEndIndex = -1;
      for (
        let j = listStartIndex + "items: ".length;
        j < contentBlock.length;
        j++
      ) {
      ...
      if (listEndIndex !== -1) {
        const listBlock = contentBlock.substring(
          listStartIndex + "items: [".length,
          listEndIndex
        );
        ...
        contentBlock =
          contentBlock.substring(0, objectStart) +
          contentBlock.substring(nextBracket + 1);
      }
    }
  }

  // Replace inline citations
  const newContentBlock = contentBlock.replace(
    /text:\s*"([^"\\]*(\\.[^"\\]*)*)"/g,
    (match, p1) => {
      let text = p1;
  ...
  if (references.length > 0) {
    ...
    for (const ref of references) {
      ...
      structuredRefs.push(
        `    { id: ${assignedId}, title: "${ref.title.replace(/"/g, '\\"')}" }`
      );
    }
  ...
  fileContent =
    ...
    newContentBlock +
    referencesString +
    fileContent.substring(contentEnd + 1);

  // Adjust currentIndex
  currentIndex =
    contentStart + newContentBlock.length + referencesString.length;
}
